# 👋 Olá, eu sou o Matheus Henrick!

🎓 **Estudante de Análise e Desenvolvimento de Sistemas**  
💼 **Perfil:** Desenvolvimento de Software (Java, JavaScript, TypeScript, Angular, Node.js)  
📍 **Localização:** São Paulo, Brasil

---

### 👨‍💻 Sobre Mim
Desenvolvedor em formação apaixonado por resolver problemas através de código limpo, estruturas de dados e boas práticas. Tenho foco na criação de interfaces modernas e reativas no **Front-end** com **Angular**, **TypeScript** e **JavaScript**, alinhado à construção de sistemas e APIs eficientes no **Back-end** com **Java** e **Node.js** integrados ao **MySQL**.

- 🎨 **Interesse:** Desenvolvimento Web, Front-end (UI/Componentização) e Lógica de Sistemas.
- 🎯 **Objetivo:** Oportunidade de **Estágio em Tecnologia**.
- ⚡ **Destaques:** Criação de aplicações CRUD reativas, jogos em terminal (Java), automações e controle de versão com Git.

---

### 🛠️ Stacks & Ferramentas

**Front-end:**
- `HTML5` | `CSS3` | `JavaScript (ES6+)` | `TypeScript` | `Angular`

**Back-end & Banco de Dados:**
- `Java` | `Node.js` | `MySQL`

**Ferramentas & Versionamento:**
- `Git` | `GitHub` | `VS Code` | `MySQL Workbench`

---

### 📌 Projetos em Destaque

- 🏦 [**Fintech CRUD (Angular)**](https://github.com/MatheusHenrick04/ProjetoAngular) — Aplicação web reativa para gestão de operações financeiras desenvolvida com Angular e TypeScript.
- ⚔️ [**Menu Interativo RPG**](https://github.com/MatheusHenrick04/MenuInterativoRPG) — Sistema em Java focado em Orientação a Objetos com seleção de classes e gestão de inventário.
- 📜 [**Guia de Comandos Git**](https://github.com/MatheusHenrick04/Comandosgit) — Repositório de consulta rápida para o fluxo de trabalho com Git e GitHub.

---

### 📬 Vamos Conectar?

- 📁 **GitHub:** [github.com/MatheusHenrick04](https://github.com/MatheusHenrick04)

---

- ### 🐍 Minhas Contribuições

![Snake animation](https://raw.githubusercontent.com/MatheusHenrick04/MatheusHenrick04/output/github-contribution-grid-snake.svg)

